...
let generate= document.getElementById('generate').addEventListener('click', actionTaken);
...
// chaining promises to call postData & updateUI functions-- advice by the reviewer
   .then((data)=>{
    ...
    .then(updateUI);// end of postData function after calling updateUI async function--edited
  });// end of .then(data) function
  //.then(updateUI); // new try-- it doesn't work
...
// assign a variablr for API Url to make it mech easier to fetch
const apiURL= baseURL+zip+apiKey; // new edit
  // fetch API url that contains baseURL , zip, apiKey
  const response= await fetch (apiURL);
  try{
      const data= await response.json();
      console.log(data);// to check the result in the console
      return data; // as required in the project rubrics.// edited
    }// end of try
    catch (error){
...
const postData= async(url='', data={})=>{ // has 2 argument the path to make postr to and a data object
   ...
   // some edited after backing again to Udacity class room
   try{
...
//this line is for test because the received data from API is undefined Now i'll all postData with my entries and see what will happen
//postData('/ourSavedData', {temp: 35, date: 'August 30', feeling: 'not good'});-- it dosen't work
